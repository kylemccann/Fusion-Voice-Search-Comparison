...
//                 invokeSaveAsDialog(concatenatedBlob);
               uploadToPHPServer(concatenatedBlob, id, phrase, nationality, accent)
                mediaRecorder.ondataavailable = function(ConcatenateBlobs) {
    			// upload each blob to PHP server
    			uploadToPHPServer(mediaRecorder.blobs);
				}
            });
            return;
        }
...
function uploadToPHPServer(blob, id, phrase, nationality, accent) {
    var file = new File([blob], id + '-' + phrase + '-' + accent +  '.wav', {
        type: 'audio/wav'
    });
    ...
    makeXMLHttpRequest('https://devweb2017.cis.strath.ac.uk/~pqb14180/Voice-Study-Desktop/save.php', formData, function() {
        var downloadURL = 'https://devweb2017.cis.strath.ac.uk/~pqb14180/Voice-Study-Desktop/uploads1/' + file.name;
