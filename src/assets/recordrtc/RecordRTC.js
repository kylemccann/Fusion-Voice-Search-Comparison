    ...
    this.startRecording = function() {
        ...
        if (!!mediaType.audio) {
            ...
            this.audioRecorder = new RecordRTC(mediaStream, {
                ...
                numberOfAudioChannels: this.numberOfAudioChannels || 1,
...
function MediaStreamRecorder(mediaStream, config) {
    ...
    if (config.type === 'audio') {
        ...
        if (!config.mimeType || config.mimeType.toString().toLowerCase().indexOf('audio') === -1) {
            config.mimeType = isChrome ? 'audio/wav' : 'audio/wav';
        }

        if (config.mimeType && config.mimeType.toString().toLowerCase() !== 'audio/wav' && !!navigator.mozGetUserMedia) {
    ...
    this.record = function() {
        ...
        if (typeof MediaRecorder.isTypeSupported === 'function' && recorderHints.mimeType) {
            if (!MediaRecorder.isTypeSupported(recorderHints.mimeType)) {
                ...
                recorderHints.mimeType = config.type === 'audio' ? 'audio/wav' : 'video/wav';
            }
        }
